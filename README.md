...
4. **Reviewer matching & four independent expert reviews** — the paper is tagged with the official PCS keyword taxonomy (Domain 2–6, Method 1–2, Users 0–2, one Primary Contribution — "a reviewer judging my work should have expertise related to…"), reviewer personas self-rate their expertise over the same taxonomy, and code computes the IDF-weighted match plus the AC's team-coverage check. Each reviewer writes blind to the others via a *draft → fact-check → sharpen* loop, with verbatim-quote anchors verified in code, plus an adversarial advisory fifth reader
...
A **Submission checklist** tab renders the CHI 2027 paper submission checklist (desk-reject risks, anonymization, content & ethics, accessibility, supplements, PCS form, after submission, on acceptance) and auto-verifies every item the run can see — template, length category and the 12,000-word rule, the PDF's metadata *Author* field, masked references, external links, keyword quotas, abstract length, LaTeX `\Description` alt text, reference list integrity — leaving PCS-side items as checkboxes.
