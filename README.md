...
1. **Desk-reject screen** — anonymization, template, completeness (hard failures stop the run, as in the real process)
2. **Reference audit** — every bibliography entry verified against **Crossref, OpenAlex, Semantic Scholar, and DBLP** (no LLM involved: an entry is *verified*, *mismatched*, or *not found* based only on database records; corrected BibTeX offered for mismatches)
3. **ADR assessment** — the Assisted Desk Reject rubric (Originality, Correctness, Novelty, Importance, Clarity of Exposition) plus the four ADR flags, ending in a simulated AC decision
4. **Four independent expert reviews** — personas derived from the paper, each writing blind to the others via a *draft → fact-check → sharpen* loop, with verbatim-quote anchors verified in code
...
- `lib/chi2027.ts` — the CHI 2027 process encoded as data (criteria, ADR flags,
  recommendation scale, decision thresholds). Another venue would be a new data module.
